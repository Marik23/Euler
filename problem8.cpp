#include <iostream>
#include <string>
#include <vector>

int main()
{
    std::string number = "";
    std::vector<std::string> numbersWithoutZeros;
    numbersWithoutZeros.push_back("");
    int numberOfConsDigits = 5;
    int numberOfConsDigitsDecr = numberOfConsDigits - 1;
    int numberDigits = number.size();
    int blockedDigits = 0;
    for (int i = 0; i < numberDigits; ++i)
    {
	if (number[i] == '0')
	{
	    blockedDigits = numberOfConsDigits + 1;
	    if (numbersWithoutZeros.back().size() < 5)
		numbersWithoutZeros.pop_back();
	    numbersWithoutZeros.push_back("");
	    continue;
	}

	// Add to new number
	numbersWithoutZeros.back() += number[i];

    }

    if (numbersWithoutZeros.back().size() < 5)
	numbersWithoutZeros.pop_back();

    int largestProduct = 0;
    int currentProduct;
    int size;
    for (std::vector<std::string>::iterator it = numbersWithoutZeros.begin(); it != numbersWithoutZeros.end(); ++it)
    {
	size = (*it).size();
	for (int i = numberOfConsDigitsDecr; i < size; ++i)
	{
	    currentProduct = 1;
	    for (int j = i - numberOfConsDigitsDecr; j <= i; ++j)
		currentProduct *= ((*it)[j] - '0');

	    if (currentProduct > largestProduct)
		largestProduct = currentProduct;
	}
    }
 
    std::cout << "Largest product: " << largestProduct << std::endl;
}
